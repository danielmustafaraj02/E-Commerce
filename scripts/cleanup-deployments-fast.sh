#!/bin/bash
# Delete all Vercel deployments except the most recent one (parallel, faster)
...
# Delete all except the first (parallel, 5 at a time)
DELETED=0
for i in "${!DEPLOYMENT_ARRAY[@]}"; do
  if [ $i -gt 0 ]; then
    DEPLOYMENT_URL=${DEPLOYMENT_ARRAY[$i]}
    {
      ...
      vercel remove "$DEPLOYMENT_URL" --yes 2>/dev/null && echo "✅ Deleted $DEPLOYMENT_URL" || echo "⚠️  Failed: $DEPLOYMENT_URL"
    } &

    # Limit to 5 parallel deletions
    if [ $((($i) % 5)) -eq 0 ]; then
      wait
    fi
  fi
...
wait
echo ""
echo "✅ All deployments deleted! Kept only the latest."
